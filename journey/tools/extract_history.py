#!/usr/bin/env python3
"""Resolve MiniQdrant's history-driven Journey snapshots from Git evidence."""
...
def patch_for_stage(manifest: HistoryManifest, number: int, *, root: Path) -> bytes:
    ...
    with tempfile.TemporaryDirectory(prefix=f"miniqdrant-patch-{number:02d}-") as raw:
        repository = Path(raw)
...
def main() -> int:
    ...
    print(f"wrote {len(manifest.stages)} MiniQdrant Stage patch/test pairs")
    return 0
