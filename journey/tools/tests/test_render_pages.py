#!/usr/bin/env python3
"""Contracts for MiniQdrant browser-native Journey lessons."""
...
class RenderPagesTest(unittest.TestCase):
    ...
    def test_all_fifteen_cards_load_with_lossless_patch_coverage(self) -> None:
        self.assertEqual([card.number for card in self.cards], list(range(1, 16)))
    ...
    def test_main_generates_bilingual_indexes_and_fifteen_pages(self) -> None:
        ...
        for root in (Path("docs/journey"), Path("docs/zh/journey")):
            ...
            self.assertEqual(len(list(root.glob("stage-*.md"))), 15)
    ...
    def test_site_exposes_three_modes_and_same_stage_language_switch(self) -> None:
        ...
        self.assertEqual(nav.count("journey/stage-"), 30)
