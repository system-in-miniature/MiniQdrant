...
??? note "文件差异：tests/acceptance/test_cli.py"
    ```diff
...
??? note "文件差异：tests/contract/test_lifecycle.py"
    ```diff
...
??? note "文件差异：tests/storage/test_manifest.py"
    ```diff
...
??? note "文件差异：src/miniqdrant/cli.py"
    ```diff
