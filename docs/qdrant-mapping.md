> **Language**: English | [简体中文](zh/qdrant-mapping.md)

# MiniQdrant ↔ Qdrant mapping

This guide maps MiniQdrant's teaching boundaries to the closest concepts in
real Qdrant. It is a conceptual reading map, not an API or source-level
compatibility claim.

## How to read the labels

- **Equivalent**: the same core invariant is taught, despite smaller formats or
  fewer operational modes.
- **Deliberately simplified**: the component plays the same architectural role,
  but removes scale, concurrency, storage-engine, or API machinery.
- **Semantically opposite**: the name or placement suggests the Qdrant
  mechanism, while the important runtime behavior goes in the other direction.
...
## Optimization and teaching surface
...
1. Start with `collection.py` for the write, view, visibility, and publication
   boundaries.
2. Read `segment/immutable.py` with `query/planner.py` to see per-segment plan
   selection.
3. Compare `index/plain.py`, `index/hnsw.py`, and `index/quantization.py`.
4. Follow `wal.py` → `frame.py` → `manifest.py` for crash recovery.
5. Finish with `optimizer/optimizer.py`, `_optimize`, and
