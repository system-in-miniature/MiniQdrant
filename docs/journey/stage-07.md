# Stage 07 · Deterministic HNSW search
...
??? note "File diff: tests/index/test_hnsw_graph.py"
    ```diff
...
??? note "File diff: tests/index/test_hnsw_recall.py"
    ```diff
...
??? note "File diff: tests/index/test_hnsw_search.py"
    ```diff
