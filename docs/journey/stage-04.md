# Stage 04 · Exact mutable segments
...
??? note "File diff: tests/contract/test_mutable_segment.py"
    ```diff
...
??? note "File diff: tests/index/test_plain.py"
    ```diff
...
??? note "File diff: src/miniqdrant/index/plain.py"
    ```diff
...
??? note "File diff: src/miniqdrant/segment/base.py"
    ```diff
