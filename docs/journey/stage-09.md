# Stage 09 · Framed write-ahead logging
...
??? note "File diff: tests/reliability/test_wal_replay.py"
    ```diff
...
??? note "File diff: tests/reliability/test_wal_tail.py"
    ```diff
...
??? note "File diff: tests/storage/test_wal_codec.py"
    ```diff
...
??? note "File diff: src/miniqdrant/persistence/fsync.py"
    ```diff
