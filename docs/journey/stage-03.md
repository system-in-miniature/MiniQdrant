# Stage 03 · Structured payload filters
...
??? note "File diff: tests/contract/test_filters.py"
    ```diff
...
??? note "File diff: src/miniqdrant/filters/ast.py"
    ```diff
...
??? note "File diff: src/miniqdrant/filters/evaluate.py"
    ```diff
