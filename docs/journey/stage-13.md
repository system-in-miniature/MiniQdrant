# Stage 13 · Atomic snapshot and restore
...
??? note "File diff: tests/acceptance/test_snapshot_roundtrip.py"
    ```diff
...
??? note "File diff: tests/reliability/test_snapshot.py"
    ```diff
...
??? note "File diff: tests/reliability/test_snapshot_restore_failure.py"
    ```diff
...
??? note "File diff: src/miniqdrant/persistence/snapshot.py"
    ```diff
