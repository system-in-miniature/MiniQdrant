# Stage 15 · Executable vector-search labs
...
??? note "File diff: src/miniqdrant/index/hnsw.py"
    ```diff
...
??? note "File diff: src/miniqdrant/labs/plan_comparison.py"
    ```diff
...
??? note "File diff: src/miniqdrant/persistence/frame.py"
    ```diff
...
??? note "File diff: src/miniqdrant/persistence/wal.py"
    ```diff
